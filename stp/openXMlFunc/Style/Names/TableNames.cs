...
namespace openXMlFunc.Style.Names
{
    public static class TableNames
    {
        public static void TableName(WordprocessingDocument doc, int chapter)
        {
            var _doc = doc.MainDocumentPart ?? doc.AddMainDocumentPart();
            var body = _doc.Document.Body;
            if(body == null)
            {
                body = new Body();
            }
            ...
            int count = 0;
            Logger.Log($"количество таблиц {tables.Count}");
            foreach( var table in tables)
            {
                count++;
                var caption = GetParagraphBeforeTable(table); 
                if(!Regex.IsMatch(caption.InnerText, @"^Таблица \d+\.\d+ – ", RegexOptions.IgnoreCase))
                {
                    logger.Logger.Log("подпись таблицы не найдена");
