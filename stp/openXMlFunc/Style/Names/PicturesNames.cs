...
namespace openXMlFunc.Style.Names
{
    public class PicturesNames
    {
        ...
        public  PicturesNames(WordprocessingDocument doc, int chapter)
        {
            _doc = doc;
            _chapter = chapter;
        }
        ...
        {
            var doc = _doc.MainDocumentPart ?? _doc.AddMainDocumentPart();
            ...
            var pictures = body.Descendants<T>().ToList();
            int count = 0;
            ...
            foreach (var picture in pictures)
            {
                ...
                if (caption == null || !Regex.IsMatch(caption.InnerText.Trim(), @"^Рисунок \d+\.\d+ –", RegexOptions.IgnoreCase))
                {
                    ...
                    Paragraph paragraph = CreateParagrapg(
                        ...
                        new Indentation() { FirstLine = 0.ToString() }
